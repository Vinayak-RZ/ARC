...
| Simulink + host-agent catalog | **archived** | [`docs/planning/LOOP_GRAPH_SIMULINK_AGENTS.md`](docs/planning/LOOP_GRAPH_SIMULINK_AGENTS.md) |
| Kernel harden (trace → trials → patches) | **this graph** | [`LOOP_GRAPH.md`](LOOP_GRAPH.md); branch `cursor/ee-kernel-harden-trace-8db3` |
...
| 2026-09-14 | Kernel harden graph: stdlib JSONL traces, 120-run graded corpus, Unagent/Improveness advisors, I1–I3 patches |

## Kernel harden graph (2026-09-14)

| Wave | Status | Notes |
|------|--------|-------|
| 0 Plan authoring | done | 22 loop plans + plan-harden PASS ([`plans/kernel-harden-loops/`](plans/kernel-harden-loops/)) |
| 1 Trace/corpus/UI | done | `runs/<id>/trace.jsonl`, 120 scenarios, UI trace + observation excerpts |
| 2 Integrate/eval/boot | done | `./scripts/validate.sh` PASS; [`docs/planning/R1_BOOT_KERNEL_HARDEN.md`](docs/planning/R1_BOOT_KERNEL_HARDEN.md) |
| 3 Host trials | done | 120/120 traced runs; [`artifacts/kernel-harden/CORPUS_INDEX.md`](artifacts/kernel-harden/CORPUS_INDEX.md) |
| 4 Critique / I1 | done | `unmatched` no longer swallows label-unverified runs |
| 5 Advisors / I2 | done | Unagent ABSTAIN (kernel is tool-shaped); Improveness held-out split |
| 6–7 Harden + docs-out | done | [`docs/CASE_STUDY_DOMAIN_KERNEL.md`](docs/CASE_STUDY_DOMAIN_KERNEL.md) |
| 8 Self-audit / I3 | done | Graded corpus; provider-absent and awaiting-confirm split out of `labeled`; [`docs/planning/AUDIT_KERNEL_HARDEN.md`](docs/planning/AUDIT_KERNEL_HARDEN.md) |
...
**Next:** none on this graph. PR `cursor/ee-kernel-harden-trace-8db3`.
