...
- Vite + React + Zustand (layout, current run, view, graph dirty). No Cordis, no DSH runtime.
...
- `@xyflow/react` (MIT) only in `ui/src/slots/canvas/`. Kernel never imports it. Graph contract: [`arc.circuit.v1.json`](../src/electrical_engineer/circuit/arc.circuit.v1.json) → `parse_graph` → `graph.json` → `netlist.cir`. Edges render as smoothstep (orthogonal) on a 16px snap grid.
...
- **CD-KICAD:** capped composition canvas, not CAD. Palette is R L C V I Gnd. Keep the cannot-do row.
