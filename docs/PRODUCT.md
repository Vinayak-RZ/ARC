# Arc

## User

Undergraduate electrical-engineering student (India first; global UG EE first-class).

## Job

Finish and understand a UG EE assignment: method, a verified number or an unverified label, visible locally.

## Done looks like

`electrical-engineer --help`; `electrical-engineer run simulate-circuit`; omitted-id classifier; `electrical-engineer mcp` `tools/list` shows 5–7 verbs including `propose_composition`; `electrical-engineer ui` on `127.0.0.1` shows evidentiary + argument; a non-circuits pack question takes a capability path or `unchecked` without auto-SPICE.

## P0 (this graph)

- Bind allowlisted capability ids to an installed provider, or fail closed to `unchecked` plus a cannot-do id
- Expose 5–7 MCP verbs including `propose_composition`; never wait on stdio
- Write two-band run artifacts and `observation.json`
- Stop `solve-explain` from minting checked ohms
- Classify omitted workflow id on the CLI-without-host path
- Give every curriculum pack a complete co-solver path (not gold depth)
- Ingest a licence-clean RAG fixture through extract → chunk → index → retrieve
- Document ChatGPT desktop against the live verbs
- Keep photo confirm-then-stop
- Show empty, error, and waiting-human UI states without minting numbers

## Later (not this graph)

- HTTP MCP
- BYOK as an in-product hosted-key flow
- C5 simulate-after-confirm
- Large gold bank
- PyPI publish
- Arc mediates MATLAB MCP and other agents so the assistant does not ingest a huge tool list

## Non-goals

- H4 Electric Pi / Cordis / DSH runtime
- H5 unique agent harness; Python specialist fan-out; Python composition interview loop
- Faculty LMS, PG public promise, civil/mechanical packs
- ChatGPT web/mobile as a host
- Binding the UI to `0.0.0.0`
- Shipping copyrighted textbooks in git
- Inventing capability or provider ids
- Replacing DESIGN-coinbase

## Honest holes

- Missing provider → exact token `unchecked` and `CD-NO-PROVIDER`
- Missing YAML row → `CD-YAML-GAP`; use `propose_composition` of capability ids
- Photo stub does not simulate
- Peer MATLAB / Copilot scalars stay `unchecked` until an EE provider recomputes them
- Gold depth stays circuits-first; other packs may legally pass as `unchecked`
