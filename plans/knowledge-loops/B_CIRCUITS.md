# Loop plan — `B_CIRCUITS` — circuits handbook
...
|-------|-------|
| **Node id** | `B_CIRCUITS` |
| **Job** | Fill encyclopedic handbook for circuits |
| **Wave** | 1 |
| **Depends on (data)** | B_SCAFFOLD schema+checker+COVERAGE units |
| **Write paths** | `knowledge/ug-ee/circuits/**` |
...
| **Max rounds** | 3 |
| **State** | [B_CIRCUITS.state.json](B_CIRCUITS.state.json) |
...
Every COVERAGE unit under circuits has notes.md (≥1500 words, required headings), questions.md (≥5 original worked Qs with Given/Find/Solution/Answer), sources.md. Kuphaldt I-II oer/ allowed.
...
|---|--------|----------|------|
| 5 | `feat(knowledge): circuits BES and network theory handbook` | knowledge/ug-ee/circuits/** | `python scripts/check_knowledge_tree.py --pack circuits` |
...
Lead updates `B_CIRCUITS.state.json`.
