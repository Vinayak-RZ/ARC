# Loop plan — `B_POWER` — power systems handbook
...
|-------|-------|
| **Node id** | `B_POWER` |
| **Job** | Fill encyclopedic handbook for power |
| **Wave** | 2 |
| **Depends on (data)** | B_SCAFFOLD schema+checker+COVERAGE units |
| **Write paths** | `knowledge/ug-ee/power/**` |
...
| **Max rounds** | 3 |
| **State** | [B_POWER.state.json](B_POWER.state.json) |
...
```text
python scripts/check_knowledge_tree.py --pack power
```
...
Every COVERAGE unit under power has notes.md (≥1500 words, required headings), questions.md (≥5 original worked Qs with Given/Find/Solution/Answer), sources.md. original-only unless CC-BY verified.
...
|---|--------|----------|------|
| 10 | `feat(knowledge): power systems handbook` | knowledge/ug-ee/power/** | `python scripts/check_knowledge_tree.py --pack power` |
...
Lead updates `B_POWER.state.json`.
