# Loop plan — `B_ELECTRONICS` — electronics handbook

> Collapsed plan for **one loop node**. Parent: [LOOP_GRAPH.md](../../LOOP_GRAPH.md).

| Field | Value |
|-------|-------|
| **Node id** | `B_ELECTRONICS` |
| **Job** | Fill encyclopedic handbook for electronics |
| **Wave** | 1 |
| **Depends on (data)** | B_SCAFFOLD schema+checker+COVERAGE units |
| **Write paths** | `knowledge/ug-ee/electronics/**` |
...
| **Max rounds** | 3 |
| **State** | [B_ELECTRONICS.state.json](B_ELECTRONICS.state.json) |
...
```text
python scripts/check_knowledge_tree.py --pack electronics
```
...
Every COVERAGE unit under electronics has notes.md (≥1500 words, required headings), questions.md (≥5 original worked Qs with Given/Find/Solution/Answer), sources.md. Kuphaldt III-IV oer/.
...
|---|--------|----------|------|
| 7 | `feat(knowledge): analog and digital electronics handbook` | knowledge/ug-ee/electronics/** | `python scripts/check_knowledge_tree.py --pack electronics` |
...
Lead updates `B_ELECTRONICS.state.json`.
